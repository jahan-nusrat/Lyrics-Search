        ...
        <div class="single-result row align-items-center mb-3 p-3">
            ...
            <div class="col-md-3 text-md-right text-center">
                <button class="btn btn-success" data-artist="${item.artist
					.name}" data-title="${item.title}" data-index="${idx}">Get Lyrics</button>
            </div>
        </div>
        `;
        }
...
function errorHandle(idx) {
    lyrics.innerHTML = `
    <p style="font-size:1.7rem" class="text-danger">Song No: ${idx}. Lyrics Not Found<br><h5>Try Another Song</h5>
    </p>
    `;
}
...
function fetchLyrics(artist, song, idx) {
    ...
    title.innerHTML = `<b>${idx}. </b><strong> ${artist}</strong> - ${song}`;
}
...
searchResult.addEventListener('click', function (event) {
    ...
    if (clickItem.tagName === 'BUTTON') {
        ...
        const idxNo = clickItem.getAttribute('data-index')
